...
J. Wang
Created on 2020-1-8
Modified by Wei-Ting Chiu in 06/20/2021
"""
import numpy as np
import torch
import time
...
def genM(n_row, n_col):
    FL = torch.float64
    M = torch.rand(n_row, n_col, dtype=FL, device = "cpu")
    r = torch.ones(n_row, dtype=FL, device=M.device)
    c = torch.ones(n_col, dtype=FL, device=M.device)
    return M, r, c

def genM_Dirichlet(n_row, n_col, alpha):
    FL = torch.float64
    M = torch.distributions.dirichlet.Dirichlet(torch.ones([n_row, n_col], dtype=FL)*alpha).sample().T
    r = torch.ones(n_row, dtype=FL, device=M.device)
    c = torch.ones(n_col, dtype=FL, device=M.device)
    return M, r, c

def genM_Normal(n_row, n_col, mean, std):
    FL = torch.float64
    M = torch.normal(torch.ones([n_row, n_col], dtype=FL)*mean, std)
    r = torch.ones(n_row, dtype=FL, device=M.device)
    c = torch.ones(n_col, dtype=FL, device=M.device)
    return M, r, c
